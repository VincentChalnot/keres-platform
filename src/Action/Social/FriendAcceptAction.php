...
/** `POST /friends/{username}/accept` (05-social.md sec 3.3 T3). */
#[AsController]
readonly class FriendAcceptAction
{
    ...
    #[Route(path: '/friends/{username}/accept', name: 'friend_accept', requirements: ['username' => '[A-Za-z0-9_-]{3,32}'], methods: ['POST'])]
    public function __invoke(string $username): JsonResponse
    {
        ...
        try {
            $this->friendshipManager->accept($user, $requester, $this->clock->now());
        ...
        return ApiResponse::ok(['friendship' => ['username' => $requester->getUsername(), 'status' => 'accepted']]);
    }
}
