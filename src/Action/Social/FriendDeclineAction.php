...
/** `POST /friends/{username}/decline` (05-social.md sec 3.3 T4, sec 3.5: silent). */
#[AsController]
readonly class FriendDeclineAction
{
    ...
    #[Route(path: '/friends/{username}/decline', name: 'friend_decline', requirements: ['username' => '[A-Za-z0-9_-]{3,32}'], methods: ['POST'])]
    public function __invoke(string $username): JsonResponse
    {
        ...
        $requester = $this->userRepository->findOneByUsernameFold($username);
        ...
        try {
            $this->friendshipManager->decline($user, $requester, $this->clock->now());
        } catch (FriendshipNotFoundException) {
            return ApiResponse::error(ApiErrorCode::FRIENDSHIP_NOT_FOUND, 'No pending friend request from that user.');
        }

        return ApiResponse::ok(['friendship' => ['username' => $requester->getUsername(), 'status' => 'declined']]);
    }
}
