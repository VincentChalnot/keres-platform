...
| `color` | `SMALLINT`, `PieceColor` | `SMALLINT` | no | - | 2nd of unique | The side. Unique is on `(game_id, color)`, **not** `(game_id, user_id)` - that is what makes hot-seat expressible (contract sec. 4.1) |
...
| `uniq_game_player_game_color` | `UNIQUE game_player (game_id, color)` | Colour lookup inside a game; also invariant 1's DB half |
...
CREATE UNIQUE INDEX uniq_game_player_game_color ON game_player (game_id, color);
...
ALTER TABLE game_player ADD CONSTRAINT chk_game_player_color CHECK (color IN (0, 1));
```
...
SELECT count(*) FROM (SELECT game_id FROM game_player
                      GROUP BY game_id HAVING count(*) <> 2 OR count(DISTINCT color) <> 2) t;
...
`game_player` (`owner_id` = the non-null `user_id` with the lowest `color`,
`is_white` = that row's `color = 0`), drop `game_player` and `created_by_id`.
...
SELECT COUNT(DISTINCT g.id) AS gamesCount,
       COUNT(DISTINCT CASE WHEN g.gameOverAt IS NOT NULL AND g.draw = false
                            AND ((gp.color = 0 AND g.whiteWins = true)
                              OR (gp.color = 1 AND g.whiteWins = false))
